...
### Что сделано:
- Bump определён как **patch**: изменения улучшают пользовательскую документацию и раздел «Помощь», но не добавляют новую функциональность и не меняют API.
- `VERSION` содержит финальную SemVer-версию `0.17.1` без `-dev`.
- `CHANGELOG.md` содержит верхнюю секцию `0.17.1` с текущей датой `2026-06-23`.
- `docs/RELEASE_NOTES.md` содержит пользовательское описание версии `0.17.1` с датой `23.06.2026`.
- **Сверка с фактическим UI.** Установлено, что живое боковое меню рендерит [frontend/src/layout/AppMenu.vue](../frontend/src/layout/AppMenu.vue) (через `AppSidebar` → [frontend/src/layouts/AppLayout.vue](../frontend/src/layouts/AppLayout.vue)); файлы `frontend/src/layouts/SidebarNav.vue` и `frontend/src/layouts/TopBar.vue` в дерево не входят (мёртвый код). Раздел «Помощь» обнаруживает статьи через `import.meta.glob('@/docs/user-guide/*.md')` с сортировкой по числовому префиксу ([frontend/src/views/HelpView.vue](../frontend/src/views/HelpView.vue)); источник — bind-mount `./docs/user-guide` ([docker-compose.yml](../docker-compose.yml)).
- **Реструктуризация и перенумерация.** Все статьи перенумерованы так, чтобы порядок в «Помощи» повторял порядок пунктов живого меню (три группы: работа с клиентами → общение → система). Бывшая единая статья «Встроенный CRM» разделена на «Контакты», «Компании», «Сделки и карточка сделки».
- **Переписаны с нуля:** телефония (переход FreeSWITCH → облако МТС Exolve: вкладки «Номер и канал», «SIP-аккаунты», «Журнал звонков», звонок в браузере); распределение заявок (теперь вкладка внутри «Команды»).
- **Обновлены:** дашборд (KPI «Менеджеры онлайн»/«Открытых сделок»/«Документы (месяц)»/«Распределено», блок «Использование»), аналитика (воронка/прогноз/причины потерь), документы (типы документооборота), мессенджер-каналы (настройка в «Настройки → Мессенджеры», email-канал, ВКонтакте), команда (4 вкладки), права ролей (+«Товары»/«Веб-формы»), уведомления (вкладка «Настройки», матрица событие×канал, привязка Telegram), настройки (логотип/таймзона/язык), подписка (планы СОЛО/КОМАНДА/СВОБОДНЫЙ, лимиты), регистрация (slug генерируется автоматически), воронки/триггеры (открытие по прямому адресу).
- **Добавлены новые статьи:** Товары и каталог, Веб-формы, Теги и сегменты, Автоматизации, Календарь и напоминания, Планы продаж, Импорт/экспорт и поиск дублей, Чаты, AI-ассистент.
- Индекс `README.md` переписан под новую структуру; все перекрёстные ссылки выверены.

### Файлы:
`docs/user-guide/*.md` (итог: `01-login`…`29-subscription` + `README.md` + `vk-channel.md`); удалены 16 файлов со старой нумерацией. Код приложения не изменялся.

### Валидация:
- Согласованность релизных файлов: `VERSION`, верхняя секция `CHANGELOG.md`, `docs/TASK_STATE.md`, `docs/DEV_LOG.md` и `docs/RELEASE_NOTES.md` ссылаются на `0.17.1`.
- `docker compose down` + `up -d --build`; `docker compose run --rm web python manage.py check` — **0 issues**.
- Контейнер `frontend` видит 31 `.md` через bind-mount; `npm run typecheck` **EXIT=0**, `npm run build` **успешно** (glob скомпилирован с новым набором).
- В собранном бандле подтверждены маркеры новых статей («МТС Exolve», «Планы продаж», «Импорт и экспорт», «AI-ассистент», «Автоматизации») — каждый в 1 чанке.
- HTTP: SPA `/app/help` → **200**, `/app/telephony` → **200**, backend `/healthz` → **200**.
- Ruff/backend-тесты/e2e по этому изменению не запускались: `.py` и `.vue` не менялись (правка только в `.md`, отдаваемых существующим glob).

### Риски:
- Визуальный QA рендера Markdown в браузере не выполнялся (в среде нет браузера) — см. KNOWN_ISSUES #17; содержимое подтверждено наличием в бандле, но не сквозным просмотром.
- Найден навигационный сирота `/app/pipelines` (нет пункта в живом меню) и мёртвые `SidebarNav.vue`/`TopBar.vue` — зафиксировано в KNOWN_ISSUES #35; правка кода в задачу не входила.
