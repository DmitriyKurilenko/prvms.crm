...
## DEC-058: Удаление внешних CRM и выделение домена «команда» в `apps/team` (2026-06-27)
**Контекст:** Внешние CRM (amoCRM/Битрикс24) проекту не нужны — продукт строится только на встроенном CRM (зафиксировано в памяти `feedback_no_external_crm`, KNOWN_ISSUES #1). При этом приложение `apps/integrations` смешивало два несвязанных домена: внешние CRM (модели `CRMConnection`/`WebhookEndpoint`/`IntegrationErrorLog`, адаптеры amoCRM/Битрикс24, OAuth/webhooks) и **ядровую модель команды** `ManagerProfile`/`ManagerDayOff`, на которую опираются распределение заявок, телефония (провижининг SIP), профили менеджеров и онбординг. Профиль менеджера нёс рудиментарные внешне-CRM-поля (`crm_connection`, `crm_user_id`, `crm_user_name`), а встроенный режим ретроспективно натягивался на эту модель. Кроме того, `CRMConnection` FK-ами прорастал в `distribution`, `telephony`, `documents`, `channels`, а `tenant.crm_mode`/`max_crm_connections`/фичи `crm_amocrm`/`crm_bitrix24` тянулись по биллингу и тенантам.
**Решение:**
- Создано отдельное TENANT-приложение `apps/team` — единственный владелец домена команды: модели `Manager` (бывш. `ManagerProfile`, без CRM-полей, с `display_name` вместо `crm_user_name`) и `TimeOff` (бывш. `ManagerDayOff`), сервис `ensure_team_members` (бывш. `ensure_builtin_manager_profiles`). Все зависимые FK (`distribution`, `telephony`) перенацелены на `team.Manager`.
- `apps/integrations` удалено целиком. Абстракция `CRMAdapter` схлопнута: остался только `BuiltinCRMAdapter` в `apps/crm/adapter.py` + хелпер `get_crm_adapter()`; `channels`/`documents`/`distribution` зовут его напрямую.
- Удалены `tenant.crm_mode` (единственный режим — встроенный), `Plan.max_crm_connections`, фичи `crm_amocrm`/`crm_bitrix24`, лимит `crm_connections`, FK `crm_connection` из `channels`/`documents`/`distribution`. Фронтенд лишился `IntegrationsView`, `api/integrations.ts`, пункта меню и маршрута «Интеграции».
- Миграции пересозданы на пустой БД (прецедент DEC-043): все миграции удалены и сгенерированы заново; посев тарифов/фич консолидирован в `billing/0003_seed_plans` (без `max_crm_connections`, без внешне-CRM-фич); системные шаблоны документов и дефолтные настройки уведомлений сеются кодом в `provision_tenant`.
**Инварианты:**
- Доменная модель команды живёт в `apps/team` и не зависит ни от CRM-интеграций, ни от распределения; новые сущности команды добавлять туда.
- Внешних CRM в продукте нет; не возвращать `crm_mode`/адаптеры/коннекторы.
**Граница достоверности:** backend 225/225 тестов, `manage.py check` 0 issues, миграции применены на чистой БД без дрейфа, ruff чисто, фронтенд typecheck/build/vitest 11/11; живой [граница] зонд: `/healthz` и `/api/docs` 200, `seed_demo_users` провизионил 10 тенантов, планы `solo/komanda/free-custom` + 12 фич засеяны, `ensure_team_members` синхронизировал 4 менеджера; **[сквозь] Playwright e2e 9/9 passed**. E2e поймал латентные ссылки на удалённый FK `crm_connection` на уровне сериализаторов (`channels/api.py`, `documents/{api,signing,esign_agreement}.py`) и косметику дисплеев фронтенда — не покрыто юнит-тестами, устранено; подробности в DEV_LOG 2026-06-27.

## DEC-057: Hermes без onboarding — OpenCode gateway config как код (2026-06-24)
**Контекст:** Интеграция AI-ассистента и резюме звонков использует Hermes через OpenAI-compatible API. По актуальной документации Hermes wizard `hermes setup` не является обязательным: рабочее состояние задаётся через `HERMES_HOME/config.yaml` и env-секреты. В проекте был риск рассинхронизации: `.env.example` упоминал `OPENCODE_API_TOKEN`, compose прокидывал `OPENCODE_API_KEY` как `OPENCODE_GO_API_KEY`, а Django отправлял `tenant.slug` в поле `model`, хотя наружный API Hermes ожидает advertised model id (`hermes-agent`), а upstream-модель задаётся в конфиге Hermes.
**Решение:**
- Конфигурация Hermes стала частью проекта: `config/hermes/config.yaml` задаёт upstream `provider: opencode-go`, `default: qwen3.6-plus`, `base_url: https://opencode.ai/zen/go/v1`.
- Добавлен init-контейнер `hermes-init`, который копирует CRM skills и идемпотентно приводит существующий named volume `hermes_profiles` к OpenCode-конфигу. Это нужно, потому что прямой bind-mount `/opt/data/config.yaml` ломает миграцию Hermes (`Device or resource busy` при атомарной замене), а read-only mount `/opt/data/skills` мешает синхронизации bundled skills.
- `hermes` добавлен и в production compose; в prod он подключён к `backend` для приватного доступа Django и к `traefik` для исходящего интернета к OpenCode, потому что `backend` — `internal: true`.
- `HERMES_MODEL` теперь означает advertised model name Hermes API (`hermes-agent`), а не upstream LLM. Upstream-модель OpenCode задаётся только в `config/hermes/config.yaml`. Django (`apps/ai_assistant/services.py`) отправляет `model=HERMES_MODEL`.
- Env-контракт унифицирован: `OPENCODE_API_KEY` (и опционально прямой `OPENCODE_GO_API_KEY` в compose), `HERMES_API_KEY`, `HERMES_API_URL=http://hermes:8642`.
**Инварианты:**
- Не запускать `hermes setup` внутри контейнера для штатного деплоя; config задаётся кодом и env.
- Не монтировать `config.yaml` поверх `/opt/data/config.yaml` напрямую; Hermes должен иметь writable `HERMES_HOME`.
- Для OpenAI-compatible запросов приложения использовать `model=hermes-agent`; `qwen3.6-plus` остаётся внутренней настройкой Hermes/OpenCode.
**Граница достоверности:** локальный Hermes API поднят через compose; `/health` и `/v1/models` отвечают 200; live `POST /v1/chat/completions` с `model=hermes-agent` через OpenCode вернул `ok`. `model=qwen3.6-plus` как внешний API model id не подходит текущему Hermes API и приводил к empty reply, поэтому исправлен на `hermes-agent`.

## DEC-056: Транскрипция и резюме звонков — Deepgram + Hermes (2026-06-24, P0 Фаза 2)
**Контекст:** Записи звонков уже скачивались (`download_call_record` → `CallRecord.record_file`, mp3), но не использовались. Цель — распознать разговор и сформировать AI-резюме в таймлайне сделки. Дорожная карта оставляла выбор ASR открытым DEC.
**Решение:**
- **ASR = Deepgram (выбран ведущим).** Контракт сверён не догадкой, а живым зондом против боевого API: `POST https://api.deepgram.com/v1/listen?model=nova-2&language=ru&smart_format=true&punctuate=true`, заголовок `Authorization: Token <DEEPGRAM_API_KEY>`, тело — сырые байты записи с `Content-Type: audio/mpeg`, ответ `results.channels[0].alternatives[0].transcript`/`.confidence` и `metadata.duration/request_id`. Реальные прогоны вернули confidence 0.998 и валидные request_id как по URL-, так и по байтовому пути (через сам клиент). Клиент изолирован в `apps/telephony/deepgram_client.py`; узкие `except` (`requests.RequestException`/`ValueError`/`KeyError`) переводятся в доменную `DeepgramError`, логирование — `apps.telephony.transcription`.
- **LLM-резюме переиспользует существующий Hermes** (`/v1/chat/completions`, провайдер OpenCode/qwen, DEC-029) через новый помощник `apps/ai_assistant/services.py::summarize_call_text` — без новой LLM-интеграции.
- **Конвейер (Celery, tenant-scoped):** `download_call_record` после сохранения mp3 ставит `transcribe_call_record` (если задан `DEEPGRAM_API_KEY`); та проверяет фичу тенанта, распознаёт, пишет `CallTranscript(status=done)` и ставит `summarize_call`; резюме сохраняется в транскрипт и кладётся `Activity(activity_type='note', related_call, deal=…)` в таймлайн сделки (`CallRecord.crm_lead_id`). Идемпотентность — статусом `CallTranscript`; повторное распознавание готовой записи пропускается.
- **Модель `CallTranscript`** (tenant, миграция `telephony/0004`): `call` (OneToOne), `status` (pending/processing/done/failed), `text`, `summary`, `language`, `confidence`, `provider`, `error`. Намеренно плоская (без посегментной диаризации) — её можно добавить полем позже без слома.
- **Фича/тариф:** `ai_call_intelligence` (миграция `billing/0009`, привязана к solo/komanda/free-custom) гейтует фоновый запуск (`Plan.has_feature`) и ручные endpoint-ы `POST /telephony/calls/{id}/transcribe/` и `GET …/transcript/` (`require_feature_access`). Несёт переменную внешнюю стоимость (Deepgram + LLM), поэтому вынесена в отдельную фичу, а не общий `telephony`.
- **Фронтенд:** в журнале звонков (`TelephonyView`) — колонка «AI-резюме» с показом резюме, статусом распознавания и кнопкой «Распознать»; готовое резюме также видно как активность в таймлайне сделки.
- **Тест-инфра:** `TenantAPITestCase._ensure_crm_plan` теперь дозаполняет фичи и для уже засеянного миграцией плана `crm` (ранний `return` не обновлял M2M), иначе новые коды `ALL_FEATURE_CODES` на тестовый план не попадали.
**Альтернативы:** (1) мультимодальный вход в LLM вместо ASR — отклонено, текущий провайдер qwen текстовый; (2) хранить резюме только в активности без модели — отклонено, нужен статус/повтор/идемпотентность; (3) общий feature `telephony` — отклонено из-за переменной стоимости внешних вызовов.
**Достоверность:** контракт Deepgram подтверждён реальными вызовами (граница). Сквозной результат (реальная запись Exolve → транскрипт → резюме в ЛК) проверяется только после деплоя этого кода на прод (там сейчас старый код); изменение самодиагностируемо — логгер пишет `request_id`/статус. Резюме идёт через Hermes (DEC-029), конфигурация модели Hermes — на стороне инфраструктуры. См. KNOWN_ISSUES #36.
