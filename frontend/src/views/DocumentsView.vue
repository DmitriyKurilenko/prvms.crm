  ...
  <FeatureGate feature="documents">
    <section class="animate-fade">
      <div class="section-header">
        <h1 class="page-title">Документы и шаблоны</h1>
      </div>

      <div class="tabs-bar">
        <button :class="['tab-btn', { active: tab === 'documents' }]" @click="tab = 'documents'">Документы</button>
        <button :class="['tab-btn', { active: tab === 'templates' }]" @click="tab = 'templates'">Шаблоны</button>
      </div>

      <!-- DOCUMENTS TAB -->
      <div v-if="tab === 'documents'" class="tab-content">
        <div class="toolbar">
          <PButton label="Сгенерировать документ" icon="pi pi-plus" size="small" @click="showGenerateForm = true" />
        </div>
        <PDataTable v-responsive-table :value="documents" size="small" stripedRows :paginator="true" :rows="20" :rowsPerPageOptions="[10, 20, 50]">
          <PColumn field="id" header="ID" />
          <PColumn field="document_type" header="Тип">
            <template #body="{ data }">{{ documentTypeLabel(data.document_type as string) }}</template>
          </PColumn>
          ...
          <PColumn header="">
            <template #body="{ data }">
              <PButton icon="pi pi-eye" text size="small" @click="previewDocument(data.id)" title="Просмотр" />
      ...
      <PDialog v-model:visible="showEditor" :header="editorIsNew ? 'Новый шаблон' : 'Редактирование шаблона'" :style="{ width: '900px', maxWidth: '95vw' }" modal>
        <div class="form-grid">
          ...
          <div>
            ...
            <PSelect v-model="editorForm.document_type" :options="documentTypeOptions" optionLabel="label" optionValue="value" class="w-full" />
          </div>
          <TemplateHtmlEditor ref="htmlEditorRef" :field-options="dealFieldOptions" />
      ...
      <PDialog v-model:visible="showGenerateForm" header="Генерация документа" :style="{ width: '450px' }" modal>
        <div class="form-grid">
          <div>
            <label class="field-label">Тип документа</label>
            <PSelect v-model="genForm.document_type" :options="documentTypeOptions" optionLabel="label" optionValue="value" class="w-full" />
          </div>
          <div>
            <label class="field-label">Шаблон *</label>
            <PSelect v-model="genForm.template_id" :options="filteredTemplateOptions" optionLabel="label" optionValue="value" placeholder="Выберите шаблон" class="w-full" />
          </div>
          <div>
          ...
          <PButton label="Сгенерировать" @click="generateDocument" :disabled="!genForm.template_id" />
        </div>
      </PDialog>
      ...
      <PDialog v-model:visible="showSendDialog" header="Отправить на подпись" :style="{ width: '500px' }" modal>
        <div class="form-grid">
          <p>Документ #{{ sendDocument?.id }} — {{ statusLabel(sendDocument?.status as string) }}</p>
          ...
          <template v-else>
            <div class="signing-link-box">
              ...
              <p style="color: var(--text-muted); font-size: 12px; margin: 4px 0 0">Скопируйте ссылку и отправьте клиенту. Клиент откроет её, запросит код подтверждения и подпишет документ.</p>
            </div>
          </template>
        </div>
      </PDialog>
      ...
      <PDialog v-model:visible="showPreview" header="Предпросмотр" :style="{ width: '700px' }" modal>
        <div v-html="previewHtml" class="document-preview" />
      </PDialog>
    </section>
  </FeatureGate>
</template>
...
import TemplateHtmlEditor from '@/components/TemplateHtmlEditor.vue'
...
const templateOptions = computed(() =>
  templates.value.filter(t => t.is_active).map(t => ({ label: `${t.name as string} (${documentTypeLabel(t.document_type as string)})`, value: t.id as number }))
)
const filteredTemplateOptions = computed(() => {
  if (!genForm.document_type) return templateOptions.value
  return templates.value
    .filter(t => t.is_active && t.document_type === genForm.document_type)
    .map(t => ({ label: t.name as string, value: t.id as number }))
})
...
const documentTypeOptions = [
  ...
  { label: 'Доп. соглашение', value: 'addendum' },
...
const load = async () => {
  templates.value = await api('/documents/templates/')
...
const editorForm = reactive({ name: '', document_type: 'contract', is_active: true })
const htmlEditorRef = ref<{ getHtml: () => string; setHtml: (html: string) => void } | null>(null)
...
const openEditor = async (tplData: Record<string, unknown> | null) => {
  if (tplData) {
    ...
    editorForm.document_type = (tplData.document_type as string) || 'contract'
    editorForm.is_active = tplData.is_active as boolean
  } else {
    editorIsNew.value = true
    editorTemplateId.value = null
    editorForm.name = ''
    editorForm.document_type = 'contract'
    editorForm.is_active = true
  }
  showEditor.value = true
  await nextTick()
  htmlEditorRef.value?.setHtml(
    tplData ? (tplData.html_body as string) || '' : '<h1>Документ</h1><p>Текст документа...</p>',
  )
}
...
const saveTemplate = async () => {
  const html = htmlEditorRef.value?.getHtml() || ''
  const variables = extractVariables(html)
  if (editorIsNew.value) {
    await api('/documents/templates/', {
      method: 'POST',
      body: { name: editorForm.name, document_type: editorForm.document_type, html_body: html, variable_schema: variables, is_active: editorForm.is_active },
    })
  } else {
    await api(`/documents/templates/${editorTemplateId.value}/`, {
      method: 'PATCH',
      body: { name: editorForm.name, document_type: editorForm.document_type, html_body: html, variable_schema: variables, is_active: editorForm.is_active },
    })
  }
  ...
  if (!editorIsNew.value && editorTemplateId.value) {
    ...
    await api(`/documents/templates/${editorTemplateId.value}/mappings/0/`, { method: 'PUT', body: rows })
  }
  await load()
}
...
const genForm = reactive({ document_type: 'contract', template_id: null as number | null, deal_id: null as number | null, signing_method: 'sms_otp' })
...
const generateDocument = async () => {
  ...
  genForm.document_type = 'contract'
  await load()
}

/* === Signing flow === */
const showSendDialog = ref(false)
const sendDocument = ref<Record<string, unknown> | null>(null)
const sendRecipient = ref('')
const signingError = ref('')
const signingUrl = ref('')

const openSendDialog = (document: Record<string, unknown>) => {
  sendDocument.value = document
  sendRecipient.value = (document.contact_phone as string) || ''
  signingError.value = ''
  signingUrl.value = (document.signing_url as string) || ''
  showSendDialog.value = true
}

const sendForSigning = async () => {
  if (!sendDocument.value || !sendRecipient.value) return
  signingError.value = ''
  try {
    const result = await api<{ detail: string; token: string; signing_url: string }>(
      `/documents/${sendDocument.value.id}/send-for-signing/`,
...
const previewDocument = async (id: number) => {
  const data = await api<{ html_snapshot: string }>(`/documents/${id}/`)
  previewHtml.value = data.html_snapshot
  showPreview.value = true
}

const previewTemplate = async (templateId: number) => {
  const data = await api<{ html: string }>(`/documents/templates/${templateId}/preview/`)
...
const downloadPdf = (id: number) => {
  ...
  window.open(`${apiUrl}/documents/${id}/pdf/?token=${token}&tenant_slug=${slug}`, '_blank')
}
