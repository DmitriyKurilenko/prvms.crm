...
def _ensure_esign_agreement(document: Document, session: SigningSession, ip_address, user_agent):
    ...
    try:
        ...
        agreement.signature_data = {
            ...
            'document_id': 0,  # will be set after save
            'note': f'Подписано автоматически при подписании документа #{document.id}',
        }
        agreement.save()
        agreement.signature_data['document_id'] = agreement.id
        ...
        Activity.objects.create(
            ...
            body=f'Соглашение #{agreement.id} подписано автоматически при подписании документа #{document.id}',
            status='done',
            created_by=document.created_by,
        )
