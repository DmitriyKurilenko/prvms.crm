...
class DocumentSigningFlowTest(TenantAPITestCase):
    def setUp(self):
        super().setUp()
        self.owner = self.create_user(role='owner', email='documents_owner@example.com', username='documents_owner')
    ...
    @patch('apps.documents.signing._send_otp')
    def test_send_for_signing_creates_session(self, _mock_send_otp):
        session = send_for_signing(self.document, 'client@example.com')
        ...
        self.assertEqual(self.document.status, 'sent')
    ...
    @patch('apps.documents.tasks.notify_document_signed.delay')
    @patch('apps.documents.signing._send_otp')
    @patch('apps.documents.signing._generate_otp', return_value='123456')
    def test_verify_success_signs_document_and_marks_lookup_used(self, _mock_generate, _mock_send_otp, mock_notify):
        session = send_for_signing(self.document, 'client@example.com')
        request_signing_otp(str(session.token))
        ...
        mock_notify.assert_called_once_with(self.tenant.id, self.document.id)

    @patch('apps.documents.signing._send_otp')
    @patch('apps.documents.signing._generate_otp', return_value='111111')
    def test_verify_marks_document_expired_when_otp_expired(self, _mock_generate, _mock_send_otp):
        session = send_for_signing(self.document, 'client@example.com')
        request_signing_otp(str(session.token))
        ...
        self.document.refresh_from_db()
        self.assertEqual(self.document.status, 'expired')
