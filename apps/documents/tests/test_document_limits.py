...
class DocumentLimitTest(TenantAPITestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_generate_document_uses_monthly_limit(self):
        with schema_context('public'):
            tenant = self.tenant.__class__.objects.get(id=self.tenant.id)
            plan = tenant.plan
            plan.max_documents_per_month = 1
            plan.save(update_fields=['max_documents_per_month'])
