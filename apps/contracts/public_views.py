...
def sign_esign_agreement(request, token: str):
    ...
    with tenant_context(tenant):
        ...
        session = SigningSession.objects.select_related('contract__deal').get(id=session.id)
...
def sign_verify(request, token: str):
    ...
    try:
        contract = verify_signing(token, code, request.META.get('REMOTE_ADDR'), request.META.get('HTTP_USER_AGENT'))
    except SigningError as exc:
        try:
            context = get_signing_context(token)
        except SigningError:
...
def sign_download_pdf(request, token: str):
    """Download signed contract PDF (public, no auth)."""
    from django_tenants.utils import tenant_context
    try:
        tenant, session = _resolve_signing_session(str(token))
    except SigningError as exc:
    ...
    with tenant_context(tenant):
        ...
        if contract.status != 'signed':
            return JsonResponse({'error': 'Договор ещё не подписан'}, status=400)
...
def sign_send_email(request, token: str):
    """Send signed contract PDF to email (public, from success page)."""
    import json
    try:
    ...
    try:
        send_signed_contract_email(str(token), email)
    except SigningError as exc:
        return JsonResponse({'error': str(exc)}, status=400)
    except Exception:
